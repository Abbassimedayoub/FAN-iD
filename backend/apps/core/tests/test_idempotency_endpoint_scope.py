...
def test_same_key_same_hash_different_endpoint_is_rejected_not_replayed(user):
    ...
    outcome = service.begin(key=key, user_id=user.pk, endpoint="/api/v1/tickets/purchase", request_hash=same_hash)
...
def test_endpoint_mismatch_rejected_even_while_in_progress(user):
    """Le rejet inter-endpoints s'applique aussi pendant une exécution IN_PROGRESS, pas seulement COMPLETED."""
