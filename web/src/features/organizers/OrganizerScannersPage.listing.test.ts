...
describe("OrganizerScannersPage listing contract", () => {
  const source = readFileSync(
    resolve(process.cwd(), "src/features/organizers/OrganizerScannersPage.tsx"),
    "utf8",
  );

  it("uses a five-item server-side paginated query", () => {
    expect(source).toContain("fetchOrganizerScannersPage({");
    expect(source).toContain("Math.ceil(scannerCount / 5)");
  ...
  it("resets page one when search or status changes", () => {
    ...
    expect(source).toContain('id="scanner-status-filter"');

    const resetOccurrences = (source.match(/setScannerPage\(1\)/g) ?? []).length;

    expect(resetOccurrences).toBeGreaterThanOrEqual(3);
  });

  it("keeps scanner actions in the paginated cards", () => {
    expect(source).toContain("onLeaveDecision={openLeaveDecision}");
