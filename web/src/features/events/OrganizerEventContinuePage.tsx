...
import {
  OrganizerShell,
} from "@/features/organizers/OrganizerShell";
...
} from "./OrganizerEventCategoriesStep";
import {
  OrganizerEventPublicationStep,
} from "./OrganizerEventPublicationStep";
import type {
  OrganizerEvent,
} from "./types";

function DraftContinuation({
  event,
}: {
    ...
    <>
      <div className="mb-6 rounded-2xl border border-[#e1e7ed] bg-white px-5 py-5 shadow-sm sm:px-7">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm font-semibold text-[#30445b]">
            Catégories & quotas
          </span>

          <span
            aria-hidden="true"
            className="hidden h-px flex-1 bg-[#dfe5eb] sm:block"
          />
      ...
      {step === 2 ? (
        <OrganizerEventCategoriesStep
          ...
          onBack={() => {
            navigate(
              `/organizer/events/${current.id}/edit`,
            );
          }}
          onSaveDraft={() => {
            navigate(
              `/organizer/events/${current.id}`,
            );
          }}
          onContinue={() => {
            setStep(3);
...
export function OrganizerEventContinuePage() {
  ...
  const breadcrumbs = (
    <div className="flex items-center gap-2 text-sm">
      ...
      <span
        aria-hidden="true"
        className="text-[#b4bdc8]"
      >
        /
      </span>

      <span className="font-semibold text-[#34465c]">
        ...
        <div className="mx-auto max-w-[1060px]">
          <div className="mb-7">
            ...
            <p className="mt-2 text-sm text-[#778596]">
              Reprenez votre brouillon, complétez les catégories puis
              publiez uniquement lorsque tout est prêt.
            </p>
          </div>
          ...
          {query.isPending ? (
            ...
            <Card className="p-8 text-center">
              ...
              <p className="mt-3 text-sm text-[#718195]">
                Seuls les événements en brouillon peuvent reprendre le
